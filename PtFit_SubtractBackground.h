// PtFit_SubtractBackground.h
// David Grund, Feb 28, 2022

Bool_t debug_PtFit = kFALSE;
...
void PtFit_SubtractBackground();
void PtFit_DoInvMassFit(Double_t fPtCutLow, Double_t fPtCutUpp, Int_t iBin);

void PtFit_SubtractBackground_main(){

    gSystem->Exec("mkdir -p Trees/" + str_subfolder + "PtFit/");
    gSystem->Exec("mkdir -p Results/" + str_subfolder + "PtFit/SubtractBackground/");
...
void PtFit_SubtractBackground()
{
    ...
    if(file){
        ...
        TString name = "Results/" + str_subfolder + "PtFit/";
...
void PtFit_DoInvMassFit(Double_t fPtCutLow, Double_t fPtCutUpp, Int_t iBin)
{
    ...
    l3->Draw();
    // Prepare path
    TString path = Form("Results/%s/PtFit/SubtractBackground/bin%i", str_subfolder.Data(), iBin);
    // Print the plots
