// STARlight_OptimalRA.h
// David Grund, Mar 05, 2022

Double_t R_A_val[13] = { 0 };
Double_t chi2NDF[13] = { 0 };

void STARlight_OptimalRA_FindMinimum(Bool_t bStopWeigh);

void STARlight_OptimalRA_main(){

    STARlight_OptimalRA_FindMinimum(kTRUE);
    STARlight_OptimalRA_FindMinimum(kFALSE);
...
void STARlight_OptimalRA_FindMinimum(Bool_t bStopWeigh)
{
    ...
    for(Int_t iRA = 0; iRA < 13; iRA++){
        ...
        TString str_in = "";
        if(!bStopWeigh) str_in = "Results/" + str_subfolder + Form("PtFit_NoBkg/OptimalRA/WeighOverAll/modRA_%.2f_chi2.txt", R_A);
        else            str_in = "Results/" + str_subfolder + Form("PtFit_NoBkg/OptimalRA/StopWeighing/modRA_%.2f_chi2.txt", R_A);
    ...
    // Print the results
    TString str_out = "";
    if(!bStopWeigh) str_out = "Results/" + str_subfolder + "PtFit_NoBkg/OptimalRA/fit_RA_WeighOverAll";
    else            str_out = "Results/" + str_subfolder + "PtFit_NoBkg/OptimalRA/fit_RA_StopWeighing";
